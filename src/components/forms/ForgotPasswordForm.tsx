...
export default function ForgotPasswordForm() {
	...
	const mutation = useMutation<void, AxiosError<{ error: string }>, TforgotPasswordSchema>({
		...
		>
			{mutation.isSuccess ? (
				<Link href="/login" className="mt-1 hover:text-blue-500">
					Пароль успешно изменён, нажмите, чтобы перейти на страницу входа
				</Link>
			) : (
				<>
                ...
                {mutation.error?.response?.data.error || 'Произошла ошибка'}
              </span>
						)}
					</div>
					<Button
